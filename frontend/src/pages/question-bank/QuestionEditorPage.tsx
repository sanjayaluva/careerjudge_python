    ...
    <div className="space-y-6 p-6">
      ...
      {isLoadingQuestion && isEditMode ? (
        ...
        <>
          ...
          {/* Simple preview — shown at the bottom of the editor for flash/passage types.
              This is a simple informational preview (not the full delivery-time simulation).
              The full interactive simulation is on the QuestionDetailPage Preview tab. */}
          {(() => {
            const isFlash =
              questionType === "MCQ_WORD_FLASH_MULTI" ||
              questionType === "MCQ_IMAGE_FLASH_MULTI" ||
              questionType === "FITB_WORD_FLASH_MULTI" ||
              questionType === "FITB_IMAGE_FLASH_MULTI";
            const isPassage = questionType === "MCQ_PASSAGE_DISPLAY_MULTI";
            const hasFlashItems = isFlash && flashItems.length > 0;
            const hasPassage = isPassage && (passageTitle || passageBody);
            if (!hasFlashItems && !hasPassage) return null;

            return (
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-5">
                <h3 className="mb-3 text-sm font-semibold text-slate-900">
                  Preview (informational — delivery order)
                </h3>

                {/* Flash items simple preview */}
                {hasFlashItems && (
                  <div className="mb-4 rounded-md border border-amber-200 bg-amber-50/50 p-3">
                    <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-amber-700">
                      Step 1: Flash items (displayed first)
                      <span className="ml-2 font-normal text-slate-500">
                        · {flashInterval || "?"} ms each · {flashCount || "?"} shown
                      </span>
                    </p>
                ...
                {/* Passage simple preview */}
                {hasPassage && (
                  <div className="mb-4 rounded-md border border-blue-200 bg-blue-50/50 p-3">
                    <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-blue-700">
                      Step 1: Passage (displayed first)
                      {displayDuration && (
                        <span className="ml-2 font-normal text-slate-500">
                          · {displayDuration}s display
                        </span>
                      )}
                    </p>
                    {passageTitle && <p className="font-semibold text-slate-900">{passageTitle}</p>}
                    {passageBody && <p className="mt-1 text-sm text-slate-700">{passageBody}</p>}
                  </div>
                )}
                ...
                <div className="rounded-md border border-slate-200 bg-white p-3">
                  <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">
                    Step 2: Question (shown after flash/passage)
                  </p>
                  <p className="text-base font-medium text-slate-900">
                    {questionText1 || "(question text not entered yet)"}
                  </p>
                  {questionText2 && <p className="mt-1 text-sm text-slate-600">{questionText2}</p>}
                </div>
              </div>
            );
          })()}
